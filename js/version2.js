/**
 * Created by lch on 2018/3/27 0027. 版本2.0
 */
var data = {
    code: '001',
    codeName: '文本',
...
$(function () {
    ...
    creatTargrt(initData);    //数据初始化--根据data生成页面
...
var initData = [
    {
        code: "001",
        codeName: "文本",
        styleName: "width:100%;font-size:20px;color:#e40000;",
        className: ["txtInner"],
        content: "文本"
    },

    {
        ...
        styleName: "width:100%;",
        className: ["imgView"],
        imgView: {url: "javascript:;", src: "img/index.png"}
    },
...

    {
        ...
        autoPlay: "true",
        interval: "5000",
        className: ["carousSwip"],
        carous: [{url: "javascript:;", src: "img/index.png"}, {url: "javascript:;", src: "img/index.png"}]
    },

    {
        ...
        styleName: "width:100%;",
        className: ["classfiy"],
        classfiy: [{url: "javascript:;", name: "整容"}, {url: "javascript:;", name: "丰胸"}, {
            url: "javascript:;",
            name: "隆鼻"
        }, {url: "javascript:;", name: "变性"}]
    },

    {
        ...
        styleName: "width:100%;",
        className: ["imgList"],
        imgList: [{url: "javascript:;", src: "img/index.png"}, {url: "javascript:;", src: "img/index.png"}]
    },

    {
        ...
        styleName: "width:100%;",
        className: ["glossary"],
        glossary: {url: "javascript:;", src: "img/index.png", title: "列表一", content: "这是列表内容"}
    },
...
function creatTargrt(obj) {
    ...
    for (var i = 0; i < jsonData.length; i++) {
        ...
        switch (code) {
            ...
            case TP_TXTINNER: //"文字"
                var html = template('txtInner', jsonData[i]);
                $(".currentPage").append(html);
                break;
            case TP_IMGVIEW: //"图片"
                var html2 = template('imgView', jsonData[i]);
                $(".currentPage").append(html2);
                break;
            case TP_BTNBUTTON: //"按钮"
                var html3 = template('btnButton', jsonData[i]);
                $(".currentPage").append(html3);
                break;
            case TP_TITLENAME: //"标题"
                var html4 = template('titleName', jsonData[i]);
                $(".currentPage").append(html4);
                break;
            case TP_CAROUSSWIP: //"轮播"
                var html5 = template('carousSwip', jsonData[i]);
                $(".currentPage").append(html5);
                carousel();
                break;
            case TP_CLASSFIY: //"分类"
                var html6 = template('classfiy', jsonData[i]);
                $(".currentPage").append(html6);
                break;
            case TP_IMGLIST: //"图片列表"
                var html7 = template('imgList', jsonData[i]);
                $(".currentPage").append(html7);
                break;
            case TP_GLOSSARY: //"图文集"
                var html8 = template('glossary', jsonData[i]);
                $(".currentPage").append(html8);
                break;
            case TP_VIDEONAME: //"视频"
                var html9 = template('videoName', jsonData[i]);
                $(".currentPage").append(html9);
                break;
...
$(function () {
    ...
    $('#saveAll').click(function () {
        ...
        var jsonData = [];
        var len = $('.rtnr .currentPage>li');   //遍历查询当前单个页面currentPage
        len.each(function () {
            var that = this;
            ...
            switch (code) {
                ...
                case TP_TITLENAME:  //标题
                    ...
                    obj = {'code': code, codeName: '标题', 'styleName': style, 'className': className, 'title': title};
                    break;
                case TP_CAROUSSWIP: //轮播
                    var carous = [];
...
function pageManager(){
    $('#tabs-1 li.page').click(function () {
        $(this).addClass('active').siblings('.active').removeClass('active');

        $('.phoneMap .currentPage').removeClass('currentPage');
        $('.phoneMap .mobile_area').hide().eq($(this).index()).show().addClass('currentPage');
    });
...
function deletePage(self){
    var index = $(self).parent('.page').index();
    ...
    $.confirm({
        ...
        onOK: function () {
            ...
            $('.phoneMap .mobile_area').eq(index).remove();
        },
        onCancel: function () {
        }
    });
...
//添加界面
function addPage(){
    var xml = '<li class="page"><span>新页面</span>' +
        '<i class="fa fa-pencil-square-o edit" aria-hidden="true" onclick="editPage(this)"></i>' +
        '<i class="fa fa-trash-o delete" aria-hidden="true" onclick="deletePage(this)"></i>' +
        '<div class="edit-input"><input type="text"/><a class="ok">确定</a><a class="no">取消</a></div></div></li>';
    $('#tabs-1 > ul').append(xml);

    var uml = '<ul class="mobile_area currentPage"></ul>';
    $('.phoneMap').find('.mobile_area').hide();
    $('.phoneMap').append(uml);
...
function deleteItem() {
    $('#jsApp .rtnr .currentPage > li').each(function () {
        $(this).hover(function () {
            $('.fa-window-close', this).remove();
            $(this).addClass('border').append('<i class="fa fa-window-close" aria-hidden="true"></i>');
            $('.fa-window-close', this).click(function () {
                $(this).parent().remove();
            })
        }, function () {
